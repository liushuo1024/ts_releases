...
void rsHandler_XYZIRT(sensor_msgs::PointCloud2 pc_msg)
{
    ...
    pc_new->width = pc->width;
    pc_new->height = pc->height;
    pc_new->points.resize(pc->points.size());
    // std::cout << "size:" << pc->points.size() << "  :" << 1800 * 16 << std::endl;
    for (int h = 0; h < pc->height; h++)
    {
        for (int w = 0; w < pc->width; w++)
        {
            pc_new->at(w, h).x = pc->at(w, h).x;
            pc_new->at(w, h).y = pc->at(w, h).y;
            pc_new->at(w, h).z = pc->at(w, h).z;
            pc_new->at(w, h).intensity = pc->at(w, h).intensity;
            pc_new->at(w, h).ring = pc->at(w, h).ring;
            // 计算相对于第一个点的相对时间
            pc_new->at(w, h).time = float(pc->at(w, h).timestamp - pc->at(0, 0).timestamp);
        }
    }
    // for (int point_id = 0; point_id < pc->points.size(); ++point_id)
    // {
    //     pc_new->points[point_id].x = pc->points[point_id].x;
    //     pc_new->points[point_id].x = pc->points[point_id].x;
    //     pc_new->points[point_id].y = pc->points[point_id].y;
    //     pc_new->points[point_id].z = pc->points[point_id].z;
    //     pc_new->points[point_id].intensity = pc->points[point_id].intensity;
    //     pc_new->points[point_id].ring = pc->points[point_id].ring;
    //     // 计算相对于第一个点的相对时间
    //     pc_new->points[point_id].time = float(pc->points[point_id].timestamp - pc->points[0].timestamp);
