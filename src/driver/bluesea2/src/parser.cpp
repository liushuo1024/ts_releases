...
#include "alarm.h"
#include <ros/ros.h>
#include <common/safe_lidar_state.h>

extern ros::Publisher safe_lidar_state_pub;
extern int lidar_id;
extern ros::ServiceClient client_eHandle_set_err_no;
#if 0
...
void set_error(int error_num)
{
	common::EHandle_set srv_set;
	srv_set.request.ErrorCode = error_num;
...
int alarmProc(unsigned char *buf, int len)
{
	...
	{
		common::safe_lidar_state msg_data;
		if (len >= (int)sizeof(LidarAlarm))
		{
			...
			if (msg->hdr.type >= 0x100)
			{
				...
				{
					msg_data.state = common::safe_lidar_state::BRAKING;
					msg_data.sensor_id = lidar_id;
					safe_lidar_state_pub.publish(msg_data);
					printf("ALARM LEVEL:ALARM  MSG TYPE:%d ZONE ACTIVE:%x\n", msg->hdr.type, msg->zone_actived);
				}
				else if (getbit(msg->hdr.data, 13) == 1)
				{

					msg_data.sensor_id = lidar_id;
					msg_data.state = common::safe_lidar_state::SLOWDOWN;
					safe_lidar_state_pub.publish(msg_data);
					printf("ALARM LEVEL:WARM  MSG TYPE:%d ZONE ACTIVE:%x\n", msg->hdr.type, msg->zone_actived);
				}
				else if (getbit(msg->hdr.data, 12) == 1)
				{
					msg_data.sensor_id = lidar_id;
					msg_data.state = common::safe_lidar_state::NORMAL;
					safe_lidar_state_pub.publish(msg_data);
			...
			else if (msg->hdr.type % 2 == 1)
			{
				...
				if (getbit(msg->hdr.data, 0) == 1)
				{
					...
					msg_data.sensor_id = lidar_id;
					msg_data.state = common::safe_lidar_state::LOW_POWER_ERROR;
					safe_lidar_state_pub.publish(msg_data);
				}
				if (getbit(msg->hdr.data, 1) == 1)
				{
					...
					msg_data.sensor_id = lidar_id;
					msg_data.state = common::safe_lidar_state::MOTOR_STALL_ERROR;
					safe_lidar_state_pub.publish(msg_data);
				}
				if (getbit(msg->hdr.data, 2) == 1)
				{
					...
					msg_data.sensor_id = lidar_id;
					msg_data.state = common::safe_lidar_state::TEMPERATURE_HIGH_ERROR;
					safe_lidar_state_pub.publish(msg_data);
				}
				if (getbit(msg->hdr.data, 3) == 1)
				{
					...
					msg_data.sensor_id = lidar_id;
					msg_data.state = common::safe_lidar_state::NETWORK_ERROR;
					safe_lidar_state_pub.publish(msg_data);
				}
				if (getbit(msg->hdr.data, 4) == 1)
				{
					...
					msg_data.sensor_id = lidar_id;
					msg_data.state = common::safe_lidar_state::NO_OUTPUT_ERROR;
					safe_lidar_state_pub.publish(msg_data);
				}
			}
			else
