...
#include <signal.h>
#include "reader.h"
#include <common/bluesea_control.h>
#include <common/bluesea_defencezone.h>
#include <common/safe_lidar_state.h>
ros::Publisher safe_lidar_state_pub;
...
bool switchZone_motor(common::bluesea_defencezone::Request &req, common::bluesea_defencezone::Response &res)
{
	char cmd[12] = {0};
	...
	{
		bool flag = SendUdpCmd2(g_reader, (char *)req.ip.c_str(), 12, cmd);
		res.Response = flag;
		return flag;
	}
	res.Response = false;
...
int main(int argc, char **argv)
{
	...
	priv_nh.param("lidar_id", lidar_id, 0);
	std::cout << "lidar_id" << lidar_id << std::endl;
	...
	client_eHandle_set_err_no = priv_nh.serviceClient<common::EHandle_set>("/eHandle_set_err_no");
	safe_lidar_state_pub = priv_nh.advertise<common::safe_lidar_state>("/safe_lidar_state", 10);
