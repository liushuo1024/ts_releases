...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type) {
  ...
  if (msg_type == MSG_INDICATION) {
    ...
    if (ctx->indication_timeout.tv_sec == 0 &&
      ...
      /* Wait for an indication (name of a received request by a server, see
       * schema) */
...
static int check_confirmation(modbus_t *ctx, uint8_t *req, uint8_t *rsp,
  ...
  if ((rsp_length == rsp_length_computed ||
    ...
    if (function != req[offset]) {
      if (ctx->debug) {
        fprintf(stderr,
                "Received function not corresponding to the request (0x%X != "
                "0x%X)\n",
                function, req[offset]);
      }
      if (ctx->error_recovery & MODBUS_ERROR_RECOVERY_PROTOCOL) {
    ...
    if (ctx->debug) {
      fprintf(stderr,
              "Message length not corresponding to the computed length (%d != "
              "%d)\n",
...
int modbus_reply(modbus_t *ctx, const uint8_t *req, int req_length,
  ...
  switch (function) {
    ...
    case MODBUS_FC_READ_DISCRETE_INPUTS: {
      ...
      int start_bits =
          is_input ? mb_mapping->start_input_bits : mb_mapping->start_bits;
      ...
      if (nb < 1 || MODBUS_MAX_READ_BITS < nb) {
        rsp_length = response_exception(
            ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_VALUE, rsp, MODBUS_TRUE,
            "Illegal nb of values %d in %s (max %d)\n", nb, name,
            MODBUS_MAX_READ_BITS);
      } else if (mapping_address < 0 || (mapping_address + nb) > nb_bits) {
        rsp_length = response_exception(
            ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_ADDRESS, rsp, MODBUS_FALSE,
    ...
    case MODBUS_FC_WRITE_SINGLE_COIL: {
      ...
      if (mapping_address < 0 || mapping_address >= mb_mapping->nb_bits) {
        ...
        if (data == 0xFF00 || data == 0x0) {
          ...
          rsp_length = response_exception(
              ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_VALUE, rsp, MODBUS_FALSE,
    ...
    case MODBUS_FC_WRITE_MULTIPLE_REGISTERS: {
      ...
      if (nb < 1 || MODBUS_MAX_WRITE_REGISTERS < nb || nb_bytes != nb * 2) {
        ...
        int i, j;
        for (i = mapping_address, j = 6; i < mapping_address + nb;
             i++, j += 2) {
          /* 6 and 7 = first value */
    ...
    case MODBUS_FC_WRITE_AND_READ_REGISTERS: {
      ...
      if (nb_write < 1 || MODBUS_MAX_WR_WRITE_REGISTERS < nb_write || nb < 1 ||
        ...
        rsp_length = response_exception(
            ...
            mapping_address < 0 ? address : address + nb,
            mapping_address_write < 0 ? address_write
    ...
    default:
      rsp_length = response_exception(
          ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_FUNCTION, rsp, MODBUS_TRUE,
  ...
  if (rc > 0) {
    ...
    for (i = offset; i < offset_end; i++) {
      ...
      for (bit = 0x01; (bit & 0xff) && (pos < nb);) {
        dest[pos++] = (temp & bit) ? MODBUS_TRUE : MODBUS_FALSE;
        bit = bit << 1;
      }
    }
  }
...
/* Reads the boolean status of bits and sets the array elements
   in the destination to MODBUS_TRUE or FALSE (single bits). */
...
int modbus_read_registers(modbus_t *ctx, int addr, int nb, uint16_t *dest) {
  ...
  status =
      read_registers(ctx, MODBUS_FC_READ_HOLDING_REGISTERS, addr, nb, dest);
  return status;
}
...
static int write_single(modbus_t *ctx, int function, int addr,
  ...
  req_length =
      ctx->backend->build_request_basis(ctx, function, addr, (int)value, req);
...
int modbus_set_error_recovery(modbus_t *ctx, uint8_t error_recovery) {
  if (ctx == NULL) {
    errno = EINVAL;
    return -1;
  }
...
/* Get the timeout interval used by the server to wait for an indication from a
 * client */
...
/* Allocates 4 arrays to store bits, input bits, registers and inputs
   ...
   The modbus_mapping_new_start_address() function shall return the new
   allocated structure if successful. Otherwise it shall return NULL and set
   errno to ENOMEM. */
