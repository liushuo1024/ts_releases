...
static int _modbus_rtu_receive(modbus_t *ctx, uint8_t *req)
{
    ...
    if (ctx_rtu->confirmation_to_ignore) {
        ...
        ctx_rtu->confirmation_to_ignore = MODBUS_FALSE;
        rc = 0;
        if (ctx->debug) {
        ...
        if (rc == 0) {
            ...
            ctx_rtu->confirmation_to_ignore = MODBUS_TRUE;
        }
    }
    return rc;
}
...
static int _modbus_rtu_connect(modbus_t *ctx)
{
    ...
    if (ctx_rtu->parity == 'N') {
        dcb.Parity = NOPARITY;
        dcb.fParity = MODBUS_FALSE;
    } else if (ctx_rtu->parity == 'E') {
        dcb.Parity = EVENPARITY;
        dcb.fParity = MODBUS_TRUE;
    } else {
        /* odd */
    ...
    dcb.fTXContinueOnXoff = MODBUS_TRUE;
    dcb.fOutX = MODBUS_FALSE;
    dcb.fInX = MODBUS_FALSE;

    /* Binary mode (it's the only supported on Windows anyway) */
    dcb.fBinary = MODBUS_TRUE;

    /* Don't want errors to be blocking */
    dcb.fAbortOnError = MODBUS_FALSE;
...
static int _modbus_rtu_flush(modbus_t *ctx)
{
    ...
    return (PurgeComm(ctx_rtu->w_ser.fd, PURGE_RXCLEAR) == MODBUS_FALSE);
#else
