 ...
 */
#ifndef amr_COMMON_INCLUDE_amr_COMMON_amr_CRC_CHECK_H_
#define amr_COMMON_INCLUDE_amr_COMMON_amr_CRC_CHECK_H_
#include <cstdint>
namespace crc {
...
inline uint16_t GetCRC16(uint8_t *data, uint32_t size) {
  uint8_t position = 0;
  uint16_t crc_word = 0xFFFFU;
  uint32_t cnt = 0;
  ...
  while (size > 0) {
    ...
    position = data[cnt];
    cnt++;
    position ^= (uint8_t)crc_word;
    crc_word >>= 8;
    crc_word ^= crc_table[position];
  }

  return crc_word;
}

inline uint16_t GetCRC16H(uint8_t *data, uint32_t size) {
  return (GetCRC16(data, size) & 0xff);
}
inline uint16_t GetCRC16L(uint8_t *data, uint32_t size) {
  return (GetCRC16(data, size) >> 8);
}

inline uint16_t GetCRC16Sum(uint8_t *data, uint16_t size, uint16_t crc_base) {
  uint8_t position = 0;
  uint32_t cnt = 0;
  ...
  while (size > 0) {
    ...
    position = data[cnt];
    cnt++;
    position ^= (uint8_t)crc_base;
    crc_base >>= 8;
    crc_base ^= crc_table[position];
  }
