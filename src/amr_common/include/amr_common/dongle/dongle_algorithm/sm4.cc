...
/******************************定义系统参数FK的取值****************************************/
static const uint32_t TBL_SYS_PARAMS[4] = {0xa3b1bac6, 0x56aa3350, 0x677d9197,
                                           0xb27022dc};
...
void four_uCh2uLong(uint8_t *in, uint32_t *out) {
  int i = 0;
  *out = 0;
  for (i = 0; i < 4; i++) *out = ((uint32_t)in[i] << (24 - i * 8)) ^ *out;
}
...
void uLong2four_uCh(uint32_t in, uint8_t *out) {
  ...
  for (i = 0; i < 4; i++) *(out + i) = (uint32_t)(in >> (24 - i * 8));
}
...
// len:数据长度(任意长度数据) key:密钥（16字节） input:输入的原始数据
// output:加密后输出数据
void encode_fun(uint8_t len, uint8_t *key, uint8_t *input, uint8_t *output) {
  int i = 0, j = 0;
  uint8_t *p = (uint8_t *)malloc(50);  //定义一个50字节缓存区
  ...
  for (i = 0; i < 32; i++)  // 32次循环迭代运算
  {
    // 5-36为32个子秘钥
    ulKeyList[i + 4] =
        ulKeyList[i] ^ func_key(ulKeyList[i + 1] ^ ulKeyList[i + 2] ^
                                ulKeyList[i + 3] ^ TBL_FIX_PARAMS[i]);
  }
  /***********************************生成32轮32位长子秘钥结束**********************************/
  ...
  for (
      j = 0; j < len / 16 + ((len % 16) ? 1 : 0);
      j++)  //进行循环加密,并将加密后数据保存（可以看出此处是以16字节为一次加密，进行循环，即若16字节则进行一次，17字节补0至32字节后进行加密两次，以此类推）
  {
    /*开始处理加密数据*/
    ...
    for (i = 0; i < 32; i++) {
      ulDataList[i + 4] =
          ulDataList[i] ^ func_data(ulDataList[i + 1] ^ ulDataList[i + 2] ^
                                    ulDataList[i + 3] ^ ulKeyList[i + 4]);
    }
    /*将加密后数据输出*/
...
void decode_fun(uint8_t len, uint8_t *key, uint8_t *input, uint8_t *output) {
  int i = 0, j = 0;
  uint32_t ulKeyTmpList[4] = {0};  //存储密钥的u32数据
  uint32_t ulKeyList[36] = {0};  //用于密钥扩展算法与系统参数FK运算后的结果存储
  uint32_t ulDataList[36] = {0};  //用于存放加密数据
    ...
    for (i = 0; i < 32; i++) {
      ulDataList[i + 4] =
          ...
          func_data(ulDataList[i + 1] ^ ulDataList[i + 2] ^ ulDataList[i + 3] ^
                    ulKeyList[35 - i]);  //与加密唯一不同的就是轮密钥的使用顺序
    }
    /*将解密后数据输出*/
    uLong2four_uCh(ulDataList[35], output + 16 * j);
    uLong2four_uCh(ulDataList[34], output + 16 * j + 4);
